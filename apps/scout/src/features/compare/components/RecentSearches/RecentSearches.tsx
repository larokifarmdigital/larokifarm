...
import { clearSearchHistory, relativeTime, useSearchHistory } from '../../lib/searchHistory';

export interface RecentSearchesProps {
  /** Callback cuando el user clica una búsqueda del histórico — el padre re-lanza la query. */
  onPick: (entry: { cn?: string; ean?: string; nombre?: string }) => void;
}
        ...
        {entries.map((e) => {
          ...
          return (
            <li key={e.key}>
              <button
                type="button"
                onClick={() => onPick({ cn: e.cn, ean: e.ean, nombre: e.nombre })}
