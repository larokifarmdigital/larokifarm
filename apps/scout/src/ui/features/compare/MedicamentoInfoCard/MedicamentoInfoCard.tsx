...
export function MedicamentoInfoCard({ cn }: MedicamentoInfoCardProps) {
  ...
  if (loading) {
    return (
      <div className="animate-pulse rounded-xl border border-teal-200/60 bg-teal-50/40 p-4">
        <div className="h-3 w-32 rounded bg-teal-200/60" />
        <div className="mt-2 h-4 w-2/3 rounded bg-teal-200/60" />
      </div>
    );
  }
    ...
    >
      <header className="mb-3 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h3 className="mt-0.5 text-base font-semibold leading-snug text-zinc-900">
            {info.nombre}
          </h3>
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            ...
            {info.nregistro && (
              <span className="inline-flex items-center gap-1 rounded-md bg-zinc-100 px-2 py-0.5 font-mono text-[10px] text-zinc-600">
                NR {info.nregistro}
              </span>
            )}
          </div>
      ...
      <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
        ...
        {info.laboratorio && (
          <div>
            ...
            <dd className="mt-0.5 text-sm text-zinc-800">{info.laboratorio}</dd>
          </div>
        )}
        ...
        {info.ean && (
          <div>
            ...
            <dd className="mt-0.5 font-mono text-sm text-zinc-800">{info.ean}</dd>
          </div>
        )}
      </dl>

      {(info.prospectoUrl || info.fichaTecnicaUrl) && (
        <div className="mt-4 flex flex-wrap gap-2 border-t border-teal-100 pt-3">
          {info.prospectoUrl && (
            <a
              href={info.prospectoUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-lg border border-teal-500 bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-teal-700"
            >
          ...
          {info.fichaTecnicaUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-xs font-semibold text-zinc-700 transition-colors hover:border-teal-400 hover:text-teal-700"
            >
              Ficha técnica
              <svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 7h10v10" /><path d="M7 17 17 7" /></svg>
            </a>
