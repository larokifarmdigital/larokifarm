  ...
  if (row.precioConfianza === 'medio') {
    return (
      <span
        ...
        title="Precio extraído por IA de la ficha. Verifica directamente en la tienda."
      >
...
function BestPriceHero({ row, moneda, savings }: { row: ComparisonRow; moneda: string; savings?: number }) {
  ...
  const inner = (
    <div className="relative overflow-hidden rounded-2xl border-2 border-emerald-400/60 bg-gradient-to-br from-emerald-500 via-teal-500 to-teal-600 p-6 text-white shadow-lg sm:p-7">
      {/* Decoración de fondo */}
...
function CompactRow({
  ...
  const inner = (
    <div className="rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-teal-400 hover:shadow-md">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="text-right">
              <div className="text-lg font-bold tabular-nums leading-none text-zinc-900">
          ...
          {clickable && (
            <div className="mt-2 text-right">
              <span className="inline-flex items-center gap-1 text-[11px] font-medium text-teal-600 hover:text-teal-700">
                Ver en tienda
  ...
  return (
    <div className="flex items-start gap-4 rounded-xl border border-zinc-200 bg-white p-5">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
          {input.cn && (
            <span className="inline-flex items-center gap-1 rounded-md bg-zinc-100 px-2 py-1 font-mono text-[11px] text-zinc-700">
          ...
          {input.nombreHint && (
            <span className="inline-flex items-center rounded-md bg-teal-50 px-2 py-1 text-[12px] font-medium italic text-teal-800">
              {input.nombreHint}
            </span>
          )}
        </div>
...
function ProductCard({ product }: { product: NonNullable<ComparisonReport['product']> }) {
  return (
    <div className="flex items-start gap-4 rounded-xl border border-teal-200 bg-gradient-to-br from-teal-50 to-white p-5">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-stretch">
        ...
        <div className="flex flex-col justify-center rounded-xl border border-zinc-200 bg-white p-4 text-center sm:min-w-[140px]">
      ...
      {okRows.length === 0 && errorRows.length > 0 && (
        <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-8 text-center">
          ...
          <p className="mx-auto mt-1.5 max-w-md text-sm text-zinc-600">
            {errorRows[0].errorMessage ??
              'No se pudo encontrar el producto en las farmacias configuradas.'}
          </p>
          <div className="mx-auto mt-4 max-w-md rounded-lg bg-white p-3 text-left text-xs text-zinc-600">
            <p className="mb-1.5 font-semibold text-zinc-800">Sugerencias:</p>
            <ul className="space-y-1 pl-4 [&_li]:list-disc [&_li]:marker:text-amber-500">
              <li>Prueba con menos palabras (ej: solo marca + tamaño).</li>
              <li>Verifica que el nombre coincide con el rótulo comercial.</li>
      ...
      {okRows.length > 0 && filteredSorted.length === 0 && (
        <div className="rounded-xl border border-dashed border-zinc-300 bg-white p-6 text-center">
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50"
          >
            Mostrar todas
          </button>
        </div>
      ...
      {okRows.length === 1 && (
        <div className="rounded-xl border border-teal-200 bg-teal-50/50 px-4 py-3 text-center text-xs text-teal-800">
      ...
      {errorRows.length > 0 && okRows.length > 0 && (
        <section className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50/70 to-white p-5">
          <header className="mb-4 flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-zinc-600">
                ...
                <strong className="text-zinc-800"> Haz clic para verificar el precio directamente.</strong>
              </p>
            </div>
          </header>
          ...
          <ul className="grid gap-2 sm:grid-cols-2">
            {errorRows.map((r) => {
              ...
              const inner = (
                <div className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-3 transition-all hover:border-amber-400 hover:shadow-sm">
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {hostname && !/google\./.test(hostname) ? (
                      <div className="truncate text-[11px] text-zinc-500">{hostname}</div>
                    ) : (
                  ...
                  {clickable && (
                    <span className="inline-flex shrink-0 items-center gap-1 rounded-md bg-amber-50 px-2.5 py-1.5 text-[11px] font-semibold text-amber-800 group-hover:bg-amber-100">
                      Verificar
