...
import type { ComparisonRow, Role } from '@/shared/core';
import { formatDate, formatNumber, formatUsd } from '@/shared/lib/format';
import { EstadoBadge } from '../EstadoBadge';

export function HistorialTable({ rows, role }: { rows: ComparisonRow[]; role: Role }) {
  if (rows.length === 0) {
    return (
            ...
            return (
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-gray-900">
                  {r.proveedor ?? <span className="text-gray-400">—</span>}
                </td>
                {showBusiness && (
