import type { ComparisonStatus } from '@/shared/core';

const STYLES: Record<ComparisonStatus, { bg: string; text: string; label: string }> = {
  OK: { bg: 'bg-green-100', text: 'text-green-800', label: 'OK' },
  DISCREPANCIAS: { bg: 'bg-yellow-100', text: 'text-yellow-800', label: 'Discrepancias' },
  ERROR: { bg: 'bg-red-100', text: 'text-red-800', label: 'Error' },
};

export function EstadoBadge({ status }: { status: ComparisonStatus }) {
